...
# 侧边栏选择菜单
page = st.sidebar.radio("请选择页面", ["主页","引言：什么是人工智能", "认识鸢尾花数据集", "将你的数据划分为训练集和测试集", "读取数据的完整代码", "模型1:KNN","分类任务的课后习题讨论","模型2:决策树","模型3:支持向量机","模型4:朴素贝叶斯","模型5:多层感知机","集成学习模型"])
# 页面0：主页
if page == "主页":
    st.title("欢迎来到主页 🎉")
# 页面1：引言    
...
# 页面2：数据展示
    ...
    st.write("#%%读入鸢尾花数据集")
    st.write("fromsklearnimportdatasets")
...
# 页面3：模型训练
    ...
    st.image("https://i.postimg.cc/Gtz9yBtT/10.png")
    st.write("机器学习中，通常将所有的数据按照8:2的比例划分为训练集和测试集。训练集用于对模型进行训练，测试集用于验证模型在不同任务中的预测性能。")
    st.subheader("【python】")
    st.write("#%%划分测试集,训练集")
    st.write("fromsklearn.model_selectionimporttrain_test_split")
    st.write("importnumpyasnp")
    st.write("indics=np.arange(feature.shape[0])#生成索引序列")
    st.write("X_train_ind,X_test_ind,X_train,X_test=train_test_split(indics,feature,test_size=0.2,random_state=42)")
    st.subheader("【说明】")
    ...
    st.write("【python】")
    st.write("Y_train=label[X_train_ind]")
    st.write("Y_test=label[X_test_ind]")
...
# 页面4：模型训练
    ...
    st.write("fromsklearn.model_selectionimporttrain_test_split")
    st.write("importnumpyasnp")
    st.write("indics=np.arange(feature.shape[0])#生成索引")
    st.write("X_train_ind,X_test_ind,X_train,X_test=train_test_split(indics,feature,test_size=0.2,random_state=42)")
    st.write("Y_train=label[X_train_ind]")
    st.write("Y_test=label[X_test_ind]")
    st.caption("【注意】#X_train_ind,X_test_ind,X_train,X_test=train_test_split(indics,feature,test_size=0.2,random_state=42)")
    st.caption("X_train1,X_test,Y_train1,Y_test1=train_test_split(feature,label,test_size=0.2,random_state=42)")
    st.caption("这种方法也可以得到Y_train和Y_test，但是输出的变量个数只能是4个，不能同时输出索引值")
    ...
    st.write("【python代码】")
    st.write("importpandasaspd")
    st.write("fromsklearn.model_selectionimporttrain_test_split")
    st.write("#读取CSV文件")
    st.write("data=pd.read_csv('iris.csv')")
    st.write("#检查数据前几行，确保正确读取")
    st.write("print(data.head())")
    ...
    st.write("#分离特征和标签")
    st.write("#特征：Sepal.Length,Sepal.Width,Petal.Length,Petal.Width")
    st.write("X=data[['Sepal.Length','Sepal.Width','Petal.Length','Petal.Width']]")
    st.write("#标签：Species")
    st.write("y=data['Species']")
    st.write("得到的结果是这样的")
    st.image("https://i.postimg.cc/zXP6xDHn/19.png")
    st.caption("这种形式是我们不熟悉的，需要进行转换")
    st.caption("特征的转换很容易")
    st.write("#将特征转换为float64类型的NumPy数组")
    st.write("X_array=X.to_numpy(dtype='float64')")
    st.write("     ")
    st.caption("标签的转换有点复杂了，这里提供两种思路")
    st.write("#将标签转换为NumPy数组")
    st.write("#方法1使用if-elif条件进行转换")
    st.write("y_array=y.to_numpy()#获得numpy类型数据")
    st.write("#创建一个空数组存储转换后的数值，与输入数组y_array形状和数据类型相同的全零数组")
    st.write("y_numeric=np.zeros_like(y_array,dtype='int64')")
    st.write("foriinrange(len(y_array)):")
    st.write("ify_array[i]=='setosa':")
    st.write("y_numeric[i]=0")
    st.write("elify_array[i]=='versicolor':")
    st.write("y_numeric[i]=1")
    st.write("elify_array[i]=='virginica':")
    st.write("y_numeric[i]=2")
    st.write("     ")
    st.write("#方法2:使用pandasfactorize")
    st.write("#y_int64,classes=pd.factorize(y)#factorize按首次出现的顺序排序")
    st.write("最后就是按照一开始的思路划分训练集和测试集")
    st.write("#划分训练集和测试集")
    st.write("#通常使用80%训练，20%测试，随机种子设为42以保证可重复性")
    st.write("X_train,X_test,y_train,y_test=train_test_split(X_array,y_numeric,test_size=0.2,random_state=42)")
    st.write("     ")
    st.write("结果已经非常好看了")
    st.image("https://i.postimg.cc/VNg1KQqn/20.png")
    st.subheader("【以上部分完整的python代码】")
    st.write("#%%从csv文件导入数据并且划分训练集和测试集")
    st.write("importpandasaspd")
    st.write("fromsklearn.model_selectionimporttrain_test_split")
    st.write("#读取CSV文件")
    st.write("data=pd.read_csv('iris.csv')")
    ...
    st.write("#方法1使用if-elif条件进行转换")
    st.write("y_array=y.to_numpy()#获得numpy类型数据")
    st.write("#创建一个空数组存储转换后的数值，与输入数组y_array​​形状和数据类型相同​​的全零数组")
    st.write("y_numeric=np.zeros_like(y_array,dtype='int64')")
    st.write("foriinrange(len(y_array)):")
    st.write("ify_array[i]=='setosa':")
    st.write("y_numeric[i]=0")
    ...
    st.write("importnumpyasnp")
    st.write("#读取txt文件")
    st.write('#假设文件名为"iris.txt"，与代码在同一目录下')
    st.write("#sep='\s+':读取空格/制表符分隔文本")
    st.write("#header=0：表示将第一行（行索引0）作为列名")
    st.write("data=pd.read_csv('iris.txt',sep='\s+',header=0)")
    st.write("#提取特征列（前4列）并转换为float类型")
    st.write("features=data.iloc[:,:4].astype(float).values")
    st.write("#提取标签列（第5列）并转换为int类型")
    st.write("#首先将字符串标签映射为数字")
    ...
    st.write("#通常使用80%训练，20%测试，随机种子设为42以保证可重复性")
    st.write("fromsklearn.model_selectionimporttrain_test_split")
    st.write("X_train,X_test,y_train,y_test=train_test_split(features,labels,test_size=0.2,random_state=42)")
...
# 页面5：模型训练
  ...
  st.write("#%%训练机器学习模型")
  st.write("#KNN")
  st.write("fromsklearn.neighborsimportKNeighborsClassifier#先调包")
  st.write("clf_KNN=KNeighborsClassifier(n_neighbors=5)#建立一个模型框架")
  st.write("clf_KNN.fit(X_train,Y_train)#代入数据训练")
  st.write("训练完毕，输出一个训练好的模型对象")
  st.image("https://i.postimg.cc/8zf3BcW9/3.png")
  st.subheader("任务2：利用训练好的分类器在测试集上输出结果")
  st.write("一行代码就可以搞定~")
  st.write("【python】")
  st.write("KNN_pred=clf_KNN.predict(X_test)")
  ...
  st.write("#输出计算结果的概率值")
  st.write("KNN_pred_proba=clf_KNN.predict_proba(X_test)")
  ...
  st.write("【python】")
  st.write("#%%计算准确率")
  st.write("#方法1：使用scikit-learn库中的accuracy_score函数来计算准确率")
  st.write("fromsklearn.metricsimportaccuracy_score")
  ...
  st.write("foriinrange(Y_test.shape[0]):")
  st.write("  ifKNN_pred[i]==Y_test[i]:")
  ...
  st.write("【python】")
  st.write("#方法3：通过混淆矩阵判断结果")
  st.write("fromsklearn.metricsimportconfusion_matrix")
  st.write("KNN_matrix=confusion_matrix(Y_test,KNN_pred)")
  st.write("#使用print函数打印文本，并在结尾不添加换行符")
  st.write("print('KNN的混淆矩阵为：',end="")")
  st.write("#使用print函数打印一个空行，以实现矩阵的另起一行显示")
  st.write("print()")
  st.write("#使用print函数打印矩阵")
  ...
  st.write("【python】")
  st.write("#计算查准率和查全率")
  st.write("#axis=1表示沿着行方向进行求和,axis=0表示按列方向进行求和")
  st.write("row_sums=np.sum(KNN_matrix,axis=1)")
  st.write("colm_sums=np.sum(KNN_matrix,axis=0)")
  st.write("print('第一种鸢尾花的查全率：',round(KNN_matrix[0,0]/row_sums[0],2))")
  st.write("print('第一种鸢尾花的查准率：',round(KNN_matrix[0,0]/colm_sums[0],2))")
  st.write("print('第二种鸢尾花的查全率：',round(KNN_matrix[1,1]/row_sums[1],2))")
  st.write("print('第二种鸢尾花的查准率：',round(KNN_matrix[1,1]/colm_sums[1],2))")
  st.write("print('第三种鸢尾花的查全率：',round(KNN_matrix[2,2]/row_sums[2],2))")
  st.write("print('第三种鸢尾花的查准率：',round(KNN_matrix[2,2]/colm_sums[2],2))")
  st.write("输出结果为：")
...
# 页面6：模型训练
    ...
    st.write("fromsklearn.datasetsimportload_iris")
    st.write("fromsklearn.model_selectionimporttrain_test_split")
    st.write("fromsklearn.preprocessingimportStandardScaler")
    st.write("fromsklearn.neighborsimportKNeighborsClassifier")
    st.write("fromsklearn.metricsimportaccuracy_score")
    st.write("iris=load_iris()")
    st.write("X_iris=iris.data")
    st.write("y_iris=iris.target")
    st.write("scaler=StandardScaler()")
    st.write("X_iris_scaled=scaler.fit_transform(X_iris)")
    st.write("X_train_iris,X_test_iris,y_train_iris,y_test_iris=train_test_split(X_iris_scaled,y_iris,test_size=0.3,random_state=0)")
    st.write("configs=[")
    st.write("{'n_neighbors':3,'metric':'minkowski','p':2},")
    st.write("{'n_neighbors':5,'metric':'minkowski','p':1},")
    st.write("{'n_neighbors':7,'metric':'euclidean'},")
    st.write("{'n_neighbors':9,'metric':'chebyshev'},")
    st.write("{'n_neighbors':11,'metric':'minkowski','p':3}")
    st.write("]")
    st.write("results_iris=[]")
    st.write("forconfiginconfigs:")
    st.write("knn=KNeighborsClassifier(**config)#**config会将字典中的键值对解包为关键字参数（KeywordArguments），等价于KNeighborsClassifier(n_neighbors=5,metric='minkowski',p=1)")
    st.write("knn.fit(X_train_iris,y_train_iris)")
    st.write("preds=knn.predict(X_test_iris)")
    st.write("acc=accuracy_score(y_test_iris,preds)")
    st.write("results_iris.append((config,acc))#这是一个列表，存储了之前循环中生成的结果。每个元素是一个元组(config,acc)")
    st.write('print("任务1：鸢尾花数据集KNN参数调整结果")')
    st.write("i=1")
    st.write("forconfig,accinresults_iris:#直接解包元组中的config和acc")
    st.write('print(f"{i}.配置:{config},准确率:{acc:.4f}")')
    st.write("i+=1")
    st.image("https://i.postimg.cc/m2CXb3hD/5.png")
    st.caption("❀结合参数含义分析结果")
    ...
    st.write("#读取excel文件")
    st.write("importpandasaspd")
    st.write("data=pd.read_excel(r'数据集/wine.xlsx')")
    st.write("data_wine=data.values")
    st.write("feature_wine=data_wine[:,1:data_wine.shape[1]]#data_wine.shape[1]=14，实际上到第13列，是个开区间")
    st.write("label_wine=data_wine[:,0]")
    st.write("#划分测试集,训练集")
    st.write("fromsklearn.model_selectionimporttrain_test_split")
    st.write("importnumpyasnp")
    st.write("indics=np.arange(data.shape[0])#生成索引")
    st.write("X_train_ind,X_test_ind,X_train,X_test=train_test_split(indics,feature_wine,test_size=0.2,random_state=42)")
    st.write("Y_train=label_wine[X_train_ind]")
    st.write("Y_test=label_wine[X_test_ind]")
    st.write("#建立模型并训练")
    st.write("fromsklearn.neighborsimportKNeighborsClassifier")
    st.write("clf_KNN=KNeighborsClassifier(n_neighbors=10)")
    st.write("clf_KNN.fit(X_train,Y_train)")
    st.write("KNN_pred=clf_KNN.predict(X_test)")
    st.write("#观察准确率")
    st.write("fromsklearn.metricsimportaccuracy_score")
    st.write("acc_KNN=accuracy_score(Y_test,KNN_pred)")
    st.write("print('KNN在红酒数据集上的准确率:',round(acc_KNN,2))")
    st.write("在输出准确率的时候，同学们还尝试了其他的方法")
    st.write("print('KNN在红酒数据集上的准确率为：{:.2f}%'.format(acc_KNN*100))")
    ...
    st.write("print('KNN在红酒数据集上的准确率为：{:.2%}'.format(acc_KNN))")
...
# 页面7：模型训练
    ...
    st.image("https://i.postimg.cc/13WSCp2w/1.png")
    ...
    st.write("#%%")
    st.write("importpandasaspd")
    st.write("fromsklearn.treeimportDecisionTreeClassifier,export_text,plot_tree")
    st.write("fromsklearn.model_selectionimporttrain_test_split")
    st.write("fromsklearn.preprocessingimportLabelEncoder")
    st.write("fromsklearn.metricsimportclassification_report,confusion_matrix,accuracy_score")
    st.write("importmatplotlib.pyplotasplt")
    st.write("#1.加载数据")
    st.write("fromsklearnimportdatasets")
    st.write("iris_datas=datasets.load_iris()")
    st.write("#2.分离特征和标签")
    st.write("feature=iris_datas.data")
    st.write("label=iris_datas.target")
    st.write("#3.划分训练集和测试集")
    st.write("X_train,X_test,y_train,y_test=train_test_split(feature,label,test_size=0.2,random_state=42)")
    ...
    st.write("#计算查准率和查全率")
    st.write("#axis=1表示沿着行方向进行求和,axis=0表示按列方向进行求和")
    st.write("row_sums=np.sum(confusion_matrix,axis=1)")
    st.write("colm_sums=np.sum(confusion_matrix,axis=0)")
    st.write("print('第一种鸢尾花的查全率：',round(confusion_matrix[0,0]/row_sums[0],2))")
    st.write("print('第一种鸢尾花的查准率：',round(confusion_matrix[0,0]/colm_sums[0],2))")
    st.write("print('第二种鸢尾花的查全率：',round(confusion_matrix[1,1]/row_sums[1],2))")
    st.write("print('第二种鸢尾花的查准率：',round(confusion_matrix[1,1]/colm_sums[1],2))")
    st.write("print('第三种鸢尾花的查全率：',round(confusion_matrix[2,2]/row_sums[2],2))")
    st.write("print('第三种鸢尾花的查准率：',round(confusion_matrix[2,2]/colm_sums[2],2))")
    ...
    st.write("class_names=iris_datas.target_names,")
    st.write("filled=True,")
    st.write("rounded=True")
    st.write(")")
    st.write("plt.title('DecisionTreeVisualization')")
    st.write("plt.show()")
    st.write("#8.输出决策规则")
    st.write("tree_rules=export_text(")
    st.write("dt_model,")
    st.write("feature_names=list(iris_datas.feature_names),")
    st.write("class_names=iris_datas.target_names,#如果没有这句，输出就是0，1，2")
    st.write(")")
    st.write("print('\n决策规则:\n',tree_rules)")
    st.write("#9.特征重要性")
    st.write("importance=pd.DataFrame({")
    st.write("'特征':iris_datas.feature_names,")
    st.write("'重要性':dt_model.feature_importances_")
    st.write("}).sort_values('重要性',ascending=False)")
    st.write("print('\n特征重要性:\n',importance)")
    ...
    st.write("【python代码】")
    st.write("#使用最优的特征进行预测对比")
    st.write("X_train_best=X_train[:,-2:]#注意决策树最少需要2个变量")
    st.write("X_test_best=X_test[:,-2:]")
...
# 页面8：模型训练
    ...
    st.write("【python】")
    st.write("#%%支持向量机SVM")
    st.write("Fromsklearn.svmimportSVC")
    st.write("clf_SVM=SVC(kernel='linear')")
    st.write("clf_SVM.fit(X_train,Y_train)")
    st.write("【提问】请尝试仿照KNN的方法，请用SVM分类器进行鸢尾花的分类")
    st.info("【python代码】")
    ...
    st.write("fromsklearn.model_selectionimporttrain_test_split")
    st.write("fromsklearn.svmimportSVC")
    st.write("fromsklearn.metricsimportclassification_report,confusion_matrix")
    st.write("fromsklearn.preprocessingimportStandardScaler")
    st.write("#1.加载数据")
    st.write("iris_datas=datasets.load_iris()")
    st.write("feature=iris_datas.data#特征数据")
    st.write("label=iris_datas.target#标签数据")
    st.write("target_names1=iris_datas.target_names#类别名称")
    st.write("feature_names=iris_datas.feature_names#特征名称")
    st.write("#2.数据预处理")
    st.write("#标准化特征数据（SVM对特征尺度敏感）")
    st.write("scaler=StandardScaler()")
    st.write("feature_scaled=scaler.fit_transform(feature)")
    st.write("#3.划分训练集和测试集（保持80%训练，20%测试）")
    st.write("X_train,X_test,y_train,y_test=train_test_split(")
    st.write("feature_scaled,label,test_size=0.2,random_state=42")
    st.write(")")
    st.write("#4.创建带RBF核的SVM模型")
    st.write("svm_model=SVC(")
    st.write("kernel='rbf',#径向基函数核")
    st.write("C=1.0,#正则化参数")
    st.write("gamma='scale',#自动设置核系数")
    st.write("probability=True,#启用概率估计")
    st.write("random_state=42#随机种子")
    st.write(")")
    st.write("#5.训练模型")
    st.write("svm_model.fit(X_train,y_train)")
    st.write("#6.模型评估")
    st.write("y_pred=svm_model.predict(X_test)")
    st.write("#7.获取预测概率")
    st.write("y_prob=svm_model.predict_proba(X_test)")
    st.write("#打印分类报告")
    st.write('print("===分类性能报告===")')
    st.write("print(classification_report(y_test,y_pred,target_names=target_names1))")
    st.write("#打印混淆矩阵")
    st.write('print("\n===混淆矩阵===")')
    st.write("print(confusion_matrix(y_test,y_pred))")
    st.image("https://i.postimg.cc/mrYYLb9S/4.png")
    st.write("fromsklearn.svmimportSVC")
    st.write("#RBF核（径向基函数核，默认）")
    st.write("svm_rbf=SVC(kernel='rbf',C=1.0,gamma='scale')")
    st.write("#线性核")
    st.write("svm_linear=SVC(kernel='linear',C=1.0)")
    st.write("#多项式核")
    st.write("svm_poly=SVC(kernel='poly',degree=3,gamma='scale',coef0=1.0)")
    st.write("#Sigmoid核")
    st.write("svm_sigmoid=SVC(kernel='sigmoid',gamma='scale',coef0=0.0)")

    st.write("【提问】请尝试比较不同的核函数，并显示不同核函数的预测结果")
    st.info("【参考代码】")
    st.write("importnumpyasnp")
    st.write("fromsklearnimportdatasets")
    st.write("fromsklearn.svmimportSVC")
    st.write("fromsklearn.model_selectionimporttrain_test_split")
    st.write("fromsklearn.preprocessingimportStandardScaler")
    st.write("fromsklearn.metricsimportaccuracy_score,classification_report")
    st.write("#加载鸢尾花数据集")
    ...
    st.write("#数据标准化")
    st.write("scaler=StandardScaler()")
    st.write("X_train=scaler.fit_transform(X_train)")
    st.write("X_test=scaler.transform(X_test)")
    st.write("#创建不同核函数的SVM模型")
    st.write("svm_rbf=SVC(kernel='rbf',C=1.0,gamma='scale')")
    st.write("svm_linear=SVC(kernel='linear',C=1.0)")
    st.write("svm_poly=SVC(kernel='poly',degree=3,gamma='scale',coef0=1.0)")
    st.write("svm_sigmoid=SVC(kernel='sigmoid',gamma='scale',coef0=0.0)")
    st.write("#定义核函数列表和模型名称")
    st.write("kernels=[svm_rbf,svm_linear,svm_poly,svm_sigmoid]#kernels列表存放着不同的SVM函数")
    st.write("kernel_names=['RBF','Linear','Polynomial','Sigmoid']")
    st.write("#评估不同核函数的性能")
    st.write("results=[]")
    st.write("forkernel,nameinzip(kernels,kernel_names):#将多个可迭代对象（像列表、元组、字符串等）中对应的元素打包成一个个元组")
    st.write("#训练模型")
    st.write("kernel.fit(X_train,y_train)")
    st.write("#预测")
    st.write("y_pred=kernel.predict(X_test)")
    st.write("#计算准确率")
    st.write("accuracy=accuracy_score(y_test,y_pred)")
    st.write("#记录结果")
    st.write("results.append({")
    st.write("'name':name,")
    st.write("'model':kernel,")
    st.write("'accuracy':accuracy,")
    st.write("'y_pred':y_pred")
    st.write(")}")
    st.write('print(f"\n{name}Kernel:")')
    st.write('print(f"Accuracy:{accuracy:.4f}")')
    st.write('print("ClassificationReport:")')
    ...
    # 转换成 DataFrame 展示
     ...
     st.dataframe(df_results)
    st.info("【高级决策*】有点复杂，可先不讲")
    st.write("#7.可视化决策边界（前两个特征）")
    st.write("defplot_2d_decision_boundary(model,X,y,feature_names):")
    st.write("#只使用前两个特征")
    st.write("X=X[:,:2]")
    st.write("model.fit(X,y)#重新训练仅使用两个特征的模型")
    st.write("#创建网格点")
    st.write("x_min,x_max=X[:,0].min()-1,X[:,0].max()+1")
    st.write("y_min,y_max=X[:,1].min()-1,X[:,1].max()+1")
    st.write("xx,yy=np.meshgrid(np.arange(x_min,x_max,0.02),")
    st.write("np.arange(y_min,y_max,0.02))")
    st.write("#预测并绘制")
    st.write("Z=model.predict(np.c_[xx.ravel(),yy.ravel()])")
    st.write("Z=Z.reshape(xx.shape)")
    st.write("plt.contourf(xx,yy,Z,alpha=0.4)")
    st.write("plt.scatter(X[:,0],X[:,1],c=y,s=20,edgecolor='k')")
    st.write("plt.xlabel(feature_names[0])")
    st.write("plt.ylabel(feature_names[1])")
    st.write('plt.title("SVM决策边界(基于前两个特征)")')
...
# 页面9：模型训练
    ...
    st.write("【创建朴素贝叶斯分类的语法】")
    st.write("clf_NB=GaussianNB()")
    st.write("【提问】请尝试仿照KNN和SVM的方法，请用朴素贝叶斯分类器进行鸢尾花的分类")
    st.info("【python代码】")
    st.write("#导入所需库")
    st.write("importnumpyasnp")
    st.write("fromsklearnimportdatasets")
    st.write("fromsklearn.model_selectionimporttrain_test_split")
    st.write("fromsklearn.naive_bayesimportGaussianNB")
    st.write("fromsklearn.metricsimportclassification_report,confusion_matrix,accuracy_score")
    st.write("fromsklearn.preprocessingimportStandardScaler")
    st.write("#1.加载数据（使用指定变量名）")
    st.write("iris_datas=datasets.load_iris()")
    st.write("feature=iris_datas.data#特征数据")
    st.write("label=iris_datas.target#标签数据")
    st.write("target_names=iris_datas.target_names#类别名称")
    st.write("feature_names=iris_datas.feature_names#特征名称")
    st.write("#2.数据预处理（标准化）")
    st.write("scaler=StandardScaler()")
    st.write("feature_scaled=scaler.fit_transform(feature)")
    st.write("#3.划分训练集和测试集（80%训练，20%测试）")
    st.write("X_train,X_test,y_train,y_test=train_test_split(")
    st.write("feature_scaled,label,test_size=0.2,random_state=42")
    st.write(")")
    st.write("#4.创建朴素贝叶斯分类器（高斯朴素贝叶斯）")
    st.write("clf_NB=GaussianNB()")
    st.write("#5.训练模型")
    st.write("clf_NB.fit(X_train,y_train)")
    st.write("#6.模型评估")
    st.write("y_pred=clf_NB.predict(X_test)")
    st.write("#7.输出评估结果")
    st.write('print("===朴素贝叶斯分类结果===")')
    st.write('print("\n分类报告:")')
    st.write("print(classification_report(y_test,y_pred,target_names=target_names))")
    st.write('print("\n混淆矩阵:")')
    st.write("print(confusion_matrix(y_test,y_pred))")
    st.write('print("\n准确率:",accuracy_score(y_test,y_pred))')
    st.write("#8.输出各类别的先验概率")
    st.write('print("\n各类别先验概率:",clf_NB.class_prior_)')
    st.write("#9.输出测试集前5个样本的预测概率")
    st.write('print("\n测试集前5个样本的预测概率:")')
...
# 页面10：模型训练
    ...
    st.info("【创建多层感知机分类的语法】")
    st.write("clf_MLP=MLPClassifier()")
    st.write("【提问】请尝试仿照KNN和SVM的方法，请用MLP分类器进行鸢尾花的分类")
    st.info("【完整代码】")
    st.write("importnumpyasnp")
    st.write("fromsklearnimportdatasets")
    st.write("fromsklearn.model_selectionimporttrain_test_split")
    st.write("fromsklearn.neural_networkimportMLPClassifier")
    st.write("fromsklearn.metricsimportclassification_report,confusion_matrix")
    st.write("fromsklearn.preprocessingimportStandardScaler")
    st.write("#1.加载数据（使用指定变量名）")
    st.write("iris_datas=datasets.load_iris()")
    ...
    st.write("feature_scaled=scaler.fit_transform(feature)")
    st.write("#3.划分训练集和测试集（80%训练，20%测试）")
    st.write("X_train,X_test,y_train,y_test=train_test_split(feature_scaled,label,test_size=0.2,random_state=42)")
    st.write("#4.创建MLP模型")
    st.write("clf_MLP=MLPClassifier(")
    st.write("hidden_layer_sizes=(10,10),")
    st.write("activation='relu',#使用ReLU激活函数")
    st.write("solver='adam',#使用Adam优化器")
    st.write("alpha=0.01,#L2正则化参数")
    st.write("batch_size='auto',#自动确定批量大小")
    st.write("learning_rate='constant',#恒定学习率")
    st.write("learning_rate_init=0.01,#初始学习率")
    st.write("max_iter=200,#最大迭代次数")
    st.write("shuffle=True,#每次迭代前洗牌数据")
    st.write("random_state=42,#随机种子")
    st.write("early_stopping=True#启用早停")
    st.write(")")
    st.write("#5.训练模型")
    st.write("clf_MLP.fit(X_train,y_train)")
    st.write("#6.模型预测")
    st.write("y_pred=clf_MLP.predict(X_test)")
    st.write("#7.获取预测概率")
    st.write("y_prob=clf_MLP.predict_proba(X_test)")
    st.write("#8.输出结果")
    st.write('print("===MLP分类结果===")')
    st.write('print("\n真实标签:",y_test)')
    st.write('print("预测标签:",y_pred)')
    st.write('print("\n===预测概率===")')
    st.write("fori,(true_label,pred_label)inenumerate(zip(y_test,y_pred)):")
    st.write('print(f"\n样本{i+1}:")')
    st.write('print(f"真实类别:{target_names2[true_label]}")')
    st.write('print(f"预测类别:{target_names2[pred_label]}")')
    st.write('print("各类别概率:")')
    st.write("forclass_idx,probinenumerate(y_prob[i]):")
    st.write('print(f"{target_names2[class_idx]}:{prob:.4f}")')
    st.write('print("\n===性能评估===")')
    st.write("print(classification_report(y_test,y_pred,target_names=target_names2))")
    st.write('print("\n混淆矩阵:")')
    st.write("print(confusion_matrix(y_test,y_pred))")
    st.write("#9.输出训练过程损失曲线")
    st.write("importmatplotlib.pyplotasplt")
    ...
    st.write("feature=iris_datas.data")
    st.write("label=iris_datas.target")
    st.write("target_names2=iris_datas.target_names")
    st.write("#2.数据预处理")
    st.write("scaler=StandardScaler()")
    st.write("feature_scaled=scaler.fit_transform(feature)")
    st.write("#3.划分训练集和测试集")
    ...
    st.write("#4.创建MLP模型")
    st.write("clf_MLP=MLPClassifier(")
    st.write("hidden_layer_sizes=(100,),")
    st.write("activation='relu',")
    st.write("solver='adam',")
    st.write("alpha=0.0001,")
    st.write("learning_rate='constant',")
    st.write("learning_rate_init=0.001,#略微提高学习率以加快收敛")
    st.write("max_iter=1,#每次只迭代1次")
    st.write("warm_start=True,#保留模型状态以便继续训练")
    st.write("shuffle=True,")
    st.write("random_state=42,")
    st.write("early_stopping=False#关闭早停以便完整记录损失")
    st.write(")")
    st.write("#5.自定义训练循环并记录损失")
    st.write("epochs=200#总训练轮数")
    st.write("train_losses=[]")
    st.write("test_losses=[]")
    st.write("forepochinrange(epochs):")
    ...
    st.write("#打印进度")
    st.write("if(epoch+1)%20==0:")
    st.write('print(f"Epoch{epoch+1}/{epochs},TrainLoss:{train_losses[-1]:.4f},TestLoss:{test_losses[-1]:.4f}")')
    st.write("#6.模型预测")
    st.write("y_pred=clf_MLP.predict(X_test)")
    st.write("y_prob=clf_MLP.predict_proba(X_test)")
    st.write("#7.输出结果")
    st.write('print("\n===性能评估===")')
    st.write("print(classification_report(y_test,y_pred,target_names=target_names2))")
    st.write("#8.绘制训练集和测试集的损失曲线")
    st.write("plt.figure(figsize=(10,6))")
    st.write("plt.plot(range(1,epochs+1),train_losses,label='TrainLoss')")
    st.write("plt.plot(range(1,epochs+1),test_losses,label='TestLoss')")
    st.write("plt.title('MLP训练集和测试集损失曲线')")
    st.write("plt.xlabel('迭代次数')")
    st.write("plt.ylabel('损失值')")
    st.write("plt.legend()")
    st.write("plt.grid(True)")
    st.write("plt.show()")
    st.write("输出效果，感觉还不错")
...
# 页面11：模型训练
    ...
    st.image("https://i.postimg.cc/qv7CWMNC/18.png")
    ...
    st.image("https://i.postimg.cc/HnHrzpGn/19.png")
    ...
    st.write("importmatplotlib.pyplotasplt")
    st.write("fromsklearn.datasetsimportload_iris")
    st.write("fromsklearn.ensembleimportRandomForestClassifier")
    st.write("fromsklearn.model_selectionimporttrain_test_split")
    st.write("fromsklearn.metricsimportclassification_report,accuracy_score")
    st.write("#1.加载数据")
    st.write("iris_datas=load_iris()")
    st.write("feature=iris_datas.data#特征数据")
    st.write("label=iris_datas.target#标签数据")
    st.write("feature_names=iris.feature_names#特征名称")
    st.write("target_names=iris.target_names#类别名称")
    st.write("#2.划分训练集和测试集（80%训练，20%测试）")
    st.write("X_train,X_test,y_train,y_test=train_test_split(feature,label,test_size=0.2,random_state=42)")
    st.write("#3.创建并训练随机森林模型")
    st.write("clf_RF=RandomForestClassifier(")
    st.write("n_estimators=100,#决策树数量")
    st.write("random_state=42,")
    st.write("max_depth=3,#控制树深度防止过拟合")
    st.write(")")
    ...
    st.write("#5.特征重要度分析")
    st.write("#获取特征重要度")
    st.write("importances=clf_RF.feature_importances_")
    st.write("std=np.std([tree.feature_importances_fortreeinrf.estimators_],axis=0)")
    st.write("#创建DataFrame方便查看")
    st.write("feature_importance=pd.DataFrame({")
    st.write("'Feature':feature_names,")
    st.write("'Importance':importances,")
    st.write("'Std':std")
    st.write("}).sort_values('Importance',ascending=False)")
    st.write('print("\n特征重要度排序:")')
    st.write("print(feature_importance)")
    st.write("#6.可视化特征重要度")
    st.write("plt.figure(figsize=(10,6))")
    st.write("plt.bar(")
    ...
    st.write(")")
    st.write("plt.xticks(range(feature.shape[1]),feature_names,rotation=45)")
    st.write('plt.xlabel("featurename")')
    st.write('plt.ylabel("featureimportance")')
    st.write('plt.title("featureimportanceofRF(std)")')
...
# 6. 可视化特征重要度
    ...
    st.image("https://i.postimg.cc/wBjM7WXy/22.png")
    st.write("具体过程为：")
    ...
    st.info("【用matlab实现adaboost方法】")
    st.write("%%")
    st.write("%Adaboost方法")
    st.write("y_train_boost=y_train_KNN;")
    st.write("y_test_boost=y_test_KNN;")
    st.write("boost=fitensemble(x_train,y_train_boost,'AdaBoostM2',100,'Tree');")
...
# 4. 模型预测与评估
    ...
    st.image("https://i.postimg.cc/BZYg6y2D/23.png")
    ...
    st.info("【用matlab实现stacking方法】")
    st.write("%%")
    st.write("%stacking方法")
    st.write("%假设只有两个基分类器")
    st.write("%第一层基分类器，使用KNN和SVM")
